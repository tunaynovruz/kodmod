...
<details>
<summary>PostgreSQL indeks yaratma və istifadə nümunəsi</summary>

```sql
-- İndekssiz (tam cədvəl skanı) - yavaş
SELECT * FROM musteriler WHERE email = 'example@mail.com';

-- İndeks yaradılması
CREATE INDEX idx_email ON musteriler(email);

-- İndekslə (birbaşa giriş) - sürətli
SELECT * FROM musteriler WHERE email = 'example@mail.com';

-- İndeks performansını yoxlama
EXPLAIN ANALYZE SELECT * FROM musteriler WHERE email = 'example@mail.com';
```
</details>
...
<details>
<summary>PostgreSQL JOIN optimallaşdırması</summary>

```sql
-- İndekslər yaratmaq
CREATE INDEX idx_musteriler_seher ON musteriler(seher);
CREATE INDEX idx_sifarisler_musteri_id ON sifarisler(musteri_id);

-- İndekslənmiş birləşmə - sürətli
SELECT m.ad, s.tarix, s.mebleg
FROM musteriler m
