...
slug: saga
tags: [mikroservislər, saga, paylanmış tranzaksiyalar, choreography, orchestration, design patterns, arxitektura]
keywords: [Saga Pattern, paylanmış tranzaksiyalar, choreography, orchestration, kompensasiya tranzaksiyaları, mikroservis patterns]
...
```mermaid
sequenceDiagram
    participant OS as Sifariş Servisi
    participant PS as Ödəniş Servisi
    participant IS as İnventar Servisi
    participant DS as Çatdırılma Servisi
    
    Note over OS,DS: Uğurlu Saga Flow
    OS->>OS: Sifariş yaradılır
    OS->>PS: SifarişYaradıldı event
    PS->>PS: Ödəniş edilir
    PS->>IS: ÖdənişTamamlandı event
    IS->>IS: Məhsullar ayrılır
    IS->>DS: MəhsullarAyrıldı event
    DS->>DS: Çatdırılma planlaşdırılır
    
    Note over OS,DS: Xəta və Kompensasiya
    PS-->>OS: ÖdənişUğursuz event
    OS->>OS: Sifariş ləğv edilir
    OS->>PS: SifarişLəğvEdildi event
```
